...
public class CommonToast {
  ...
  public CommonToast(Activity activity, String message, int icon, String action, int actionIcon, long l) {
    _duration = l;
    init(activity);
    setMessage(message);
    setMessageIc(icon);
  ...
  public void show() {
    final ViewGroup content = (ViewGroup) ((Activity) _toastVw.getContext()).findViewById(android.R.id.content);
    if (content != null) {
      ...
      Logger.error("Toast not shown! Content view is null!");
    }
  }
  ...
  private class ToastView extends FrameLayout {
    ...
    private void init() {
      LayoutInflater.from(getContext()).inflate(R.layout.oc_view_base_toast, this, true);
